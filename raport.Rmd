...
Liczba zainteresowanych nią osób to: `r df1$liczba_osob_zainteresowanych[1]`
...
Nieopłacalna jest za to wycieczka do miasta `r data2$wycieczka[data2$oplacalnosc=="nieoplacalna"]`.
...
kable(data3, caption = "Liczba osób zainteresowanych na poszczególne wycieczki", align = "l") %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "hover"))
```
Jak wynika z tabeli, najwięcej (`r data3$procent_powrotow[1]`%) klientów powraca 
do miasta `r data3$wycieczka[1]`. Najmniej osób (`r tail(data3$procent_powrotow,1)`%)
...
kable(data4, caption = "Najpopularniejsze kombinacje dwóch wycieczek odwiedzanych przez tych samych klientów", align = "l") %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "hover"))
```
Można zauważyć, że najwięcej klientów wybiera `r data4$wycieczka_1[1]`
oraz `r data4$wycieczka_2[1]`.


## Wynik 5 - średnia liczba

Zwróćmy ug=wagę na to, ile osób bliskich deklarują nasi klienci.
...
Jak można zauważyć, średnia liczba osób bliskich, którą deklarował pojecynczy klient wyosi: `r data5$srednia_liczba_bliskich`.
